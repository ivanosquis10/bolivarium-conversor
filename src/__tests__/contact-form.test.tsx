...
describe("Contact Form Component", () => {
  ...
  function verifyInputValues(label: string, placeholder: string, inputValue?: string) {
    const field = {
      name: screen.getByLabelText(label),
      input: screen.getByPlaceholderText(placeholder),
    } // Obtenemos el label y el input correspondiente

    expect(field.name).toBeInTheDocument()

    if (inputValue !== undefined) {
      // Si el valor del input es distinto de undefined
  ...
  it.each([
    ...
    [
      "MESSAGE",
      "label_message",
      "Aqui escribe tu duda o sugerencia :)",
      "Me gusta mucho tu aplicacion, la verdad es la mejor que es visto uwu",
    ],
  ])(
    "Verifica si permite al usuario escribir en el input %s",
    (_, label, placeholder, inputValues) => {
